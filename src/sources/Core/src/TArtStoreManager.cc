...
void TArtStoreManager::AddParameters(TNamed *obj) {
  ...
  if(check != NULL){
    TArtCore::Warning(__FILE__,"do not make duplicate named parameters. not added : %s", obj->GetName());
    return;
  }
...
void TArtStoreManager::AddDataContainer(TArtRawEventObject *obj) {
  ...
  if(check != NULL){
    TArtCore::Warning(__FILE__,"do not make duplicate named data container. not added : %s", obj->GetName());
    return;
  }
...
void TArtStoreManager::LoadDataFromTree(TTree *tree)
{
  ...
  for (Int_t i=0; i<branchArray->GetEntriesFast(); i++) {      
...
TNamed * TArtStoreManager::RemoveDataContainer(const char* name){
  TNamed * p = (TNamed *)listOfDataContainers->FindObject(name);
  if(NULL==p){
    TArtCore::Debug(__FILE__,"Could not find data container: %s", name);
  }
  else{
    listOfDataContainers->Remove(p);
  }
  return p;
}
