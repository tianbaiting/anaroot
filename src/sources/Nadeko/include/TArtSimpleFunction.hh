...
public:
  static void ReadObjectFromFile(const char* filename);
  static void WriteObjectToFile(const char* filename);
  //write current hist to text.
  static TH1* WriteHistToText(const char* filename);
  static TH1* CreateHistFromText(const char* filename);
  static TGraph* CreateGraphFromText(const char* filename);
};
