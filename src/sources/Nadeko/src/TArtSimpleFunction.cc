...
void TArtSimpleFunction::GraphTune(TGraph* graph, const char* title, int nxdiv, int nydiv, double xmin, double xmax, double ymin, double ymax)
{
  if(!graph) return;
...
void TArtSimpleFunction::ReadObjectFromFile(const char* filename)
{
...
void TArtSimpleFunction::WriteObjectToFile(const char* filename)
{
  if(!gDirectory) return;
  TDirectory* lastdirectory = gDirectory;  
...
TH1* TArtSimpleFunction::WriteHistToText(const char* filename)
{
  TH1* hist = gH1;
  if(!hist) return 0;
...
TH1* TArtSimpleFunction::CreateHistFromText(const char* filename)
{
  std::ifstream ifs;
...
TGraph* TArtSimpleFunction::CreateGraphFromText(const char* filename)
{
  std::ifstream ifs;
  ifs.open(filename, std::ios::in);
  if(!ifs){
