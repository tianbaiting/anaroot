...
void hn(){cdn();hhn();}
void hb(){cdn();hhb();}
int hid(){if(gH1) return TArtHistManager::FindId(gH1); return 0;}
...
void graphtune(TGraph* graph, const char* title, int nxdiv, int nydiv, double xmin, double xmax, double ymin, double ymax){TArtSimpleFunction::GraphTune(graph, title, nxdiv, nydiv, xmin, xmax, ymin, ymax);}
