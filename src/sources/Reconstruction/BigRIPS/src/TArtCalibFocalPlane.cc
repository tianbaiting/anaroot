...
TArtCalibFocalPlane::TArtCalibFocalPlane(const TString outdataname)
...
TArtCalibFocalPlane::~TArtCalibFocalPlane()  {
  ...
  for(int i=0;i<(int)fPPACArrayBuffer.size();i++){
...
void TArtCalibFocalPlane::ReconstructData()   { // call after the raw data are loaded
  ...
  for(int i=0;i<GetNumFocalPlane();i++){
    ...
    for(int j=0;j<(int)ppac_array.size();j++)
