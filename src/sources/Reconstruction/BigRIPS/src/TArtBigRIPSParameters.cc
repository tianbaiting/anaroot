...
TArtPPACPara *TArtBigRIPSParameters::ParsePPACPara(TXMLNode *node) {
  ...
  Int_t     id = -1;
  TString name;
  Int_t     fpl = 0, dev = -1, t_det=-1, q_det=-1;
  ...
  Int_t Tx1_dev=-1, Tx1_fpl=0; 
  Int_t Tx2_dev=-1, Tx2_fpl=0;
  Int_t Ty1_dev=-1, Ty1_fpl=0;
  Int_t Ty2_dev=-1, Ty2_fpl=0;
  Int_t Ta_dev =-1, Ta_fpl =0;
  Int_t Qx1_dev=-1, Qx1_fpl=0; 
  Int_t Qx2_dev=-1, Qx2_fpl=0;
  Int_t Qy1_dev=-1, Qy1_fpl=0;
  Int_t Qy2_dev=-1, Qy2_fpl=0;
  Int_t Qa_dev =-1, Qa_fpl =0;

  Int_t Tx1_geo = -1, Tx2_geo = -1, Ty1_geo = -1, Ty2_geo = -1, Ta_geo = -1;
  Int_t Qx1_geo = -1, Qx2_geo = -1, Qy1_geo = -1, Qy2_geo = -1, Qa_geo = -1;
  Int_t Tx1_ch = -1, Tx2_ch = -1, Ty1_ch = -1, Ty2_ch = -1, Ta_ch = -1;
  Int_t Qx1_ch = -1, Qx2_ch = -1, Qy1_ch = -1, Qy2_ch = -1, Qa_ch = -1;
  ...
  for ( ; node; node = node->GetNextNode()) {
	...
	name = node->GetText();
      if (strcmp(node->GetNodeName(), "DEV") == 0) 
	dev = atoi(node->GetText());
	...
	t_det = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "Tx1_dev") == 0) 
	Tx1_dev = atoi(node->GetText());
	...
	Tx1_ch = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "Tx2_dev") == 0) 
	Tx2_dev = atoi(node->GetText());
	...
	Tx2_ch = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "Ty1_dev") == 0) 
	Ty1_dev = atoi(node->GetText());
	...
	Ty1_ch = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "Ty2_dev") == 0) 
	Ty2_dev = atoi(node->GetText());
	...
	Ty2_ch = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "Ta_dev") == 0) 
	Ta_dev = atoi(node->GetText());
	...
	q_det = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "Qx1_dev") == 0) 
	Qx1_dev = atoi(node->GetText());
	...
	Qx1_ch = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "Qx2_dev") == 0) 
	Qx2_dev = atoi(node->GetText());
	...
	Qx2_ch = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "Qy1_dev") == 0) 
	Qy1_dev = atoi(node->GetText());
	...
	Qy1_ch = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "Qy2_dev") == 0) 
	Qy2_dev = atoi(node->GetText());
	...
	Qy2_ch = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "Qa_dev") == 0) 
	Qa_dev = atoi(node->GetText());
  ...
  para->SetTx1Map( Tx1_dev>0 ? Tx1_dev : dev,
		   Tx1_fpl>0 ? Tx1_fpl : fpl, 
                   t_det>0 ? t_det : PPACT, Tx1_geo, Tx1_ch);
  para->SetTx2Map( Tx2_dev>0 ? Tx2_dev : dev,
		   Tx2_fpl>0 ? Tx2_fpl : fpl, 
                   t_det>0 ? t_det : PPACT, Tx2_geo, Tx2_ch);
  para->SetTy1Map( Ty1_dev>0 ? Ty1_dev : dev, 
		   Ty1_fpl>0 ? Ty1_fpl : fpl, 
                   t_det>0 ? t_det : PPACT, Ty1_geo, Ty1_ch);
  para->SetTy2Map( Ty2_dev>0 ? Ty2_dev : dev, 
		   Ty2_fpl>0 ? Ty2_fpl : fpl, 
                   t_det>0 ? t_det : PPACT, Ty2_geo, Ty2_ch);
  para->SetTaMap ( Ta_dev>0 ? Ta_dev : dev,  
		   Ta_fpl >0 ? Ta_fpl  : fpl, 
                   t_det>0 ? t_det : PPACT, Ta_geo, Ta_ch);
  para->SetQx1Map( Qx1_dev>0 ? Qx1_dev : dev,
		   Qx1_fpl>0 ? Qx1_fpl : fpl, 
                   q_det>0 ? q_det : PPACQ, Qx1_geo, Qx1_ch);
  para->SetQx2Map( Qx2_dev>0 ? Qx2_dev : dev,
		   Qx2_fpl>0 ? Qx2_fpl : fpl,  
                   q_det>0 ? q_det : PPACQ, Qx2_geo, Qx2_ch);
  para->SetQy1Map( Qy1_dev>0 ? Qy1_dev : dev, 
		   Qy1_fpl>0 ? Qy1_fpl : fpl,  
                   q_det>0 ? q_det : PPACQ, Qy1_geo, Qy1_ch);
  para->SetQy2Map( Qy2_dev>0 ? Qy2_dev : dev, 
		   Qy2_fpl>0 ? Qy2_fpl : fpl,  
                   q_det>0 ? q_det : PPACQ, Qy2_geo, Qy2_ch);
  para->SetQaMap ( Qa_dev>0 ? Qa_dev : dev,  
		   Qa_fpl >0 ? Qa_fpl  : fpl,  
                   q_det>0 ? q_det : PPACQ, Qa_geo, Qa_ch);
...
TArtPlasticPara *TArtBigRIPSParameters::ParsePlasticPara(TXMLNode *node) {
  ...
  Double_t  qtc_overflow = 10000000;
  Double_t  multdc_underflow = 0;
  Double_t  multdc_overflow = 10000000;
  Double_t  zpos = 0;

  Int_t t_dev=-1, t_fpl=-1, t_det=-1;
  Int_t tleft_geo=-1, tleft_ch=-1, tright_geo=-1, tright_ch=-1;
  Int_t q_dev=-1, q_fpl=-1, q_det=-1;
  Int_t qleft_geo=-1, qleft_ch=-1, qright_geo=-1, qright_ch=-1;
  Int_t qtc_dev=-1, qtc_fpl=-1, qtc_det=-1;
  Int_t qtcleft_geo=-1, qtcleft_ch=-1, qtcright_geo=-1, qtcright_ch=-1;
  Int_t mult_dev=-1, mult_fpl=-1, mult_det=-1;
  Int_t multleft_geo=-1, multleft_ch=-1, multright_geo=-1, multright_ch=-1;
  Bool_t findTDCoverflow = false;
  ...
  for ( ; node; node = node->GetNextNode()) {
	...
	findTDCoverflow = true;
      ...
      if (strcmp(node->GetNodeName(), "qtc_underflow") == 0)
	qtc_underflow = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(), "qtc_overflow") == 0)
	qtc_overflow = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(), "multdc_underflow") == 0)
	multdc_underflow = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(), "multdc_overflow") == 0)
	multdc_overflow = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(), "zpos") == 0)
	zpos = (Double_t)atof(node->GetText());

      if (strcmp(node->GetNodeName(), "t_dev") == 0) 
	t_dev = atoi(node->GetText());
	...
	tright_ch = atoi(node->GetText());
      ...
      if (strcmp(node->GetNodeName(), "q_dev") == 0) 
	q_dev = atoi(node->GetText());
	...
	qright_ch = atoi(node->GetText());
      ...
      if (strcmp(node->GetNodeName(), "qtc_dev") == 0) 
	qtc_dev = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "qtc_fpl") == 0) 
	qtc_fpl = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "qtc_det") == 0) 
	qtc_det = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "qtcleft_geo") == 0) 
	qtcleft_geo = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "qtcleft_ch") == 0) 
	qtcleft_ch = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "qtcright_geo") == 0) 
	qtcright_geo = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "qtcright_ch") == 0) 
	qtcright_ch = atoi(node->GetText());

      if (strcmp(node->GetNodeName(), "mult_dev") == 0) 
	mult_dev = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "mult_fpl") == 0) 
	mult_fpl = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "mult_det") == 0) 
	mult_det = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "multleft_geo") == 0) 
	multleft_geo = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "multleft_ch") == 0) 
	multleft_ch = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "multright_geo") == 0) 
	multright_geo = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "multright_ch") == 0) 
	multright_ch = atoi(node->GetText());
  ...
  para->SetMap(t_dev>0 ? t_dev : dev,
	       t_fpl>0 ? t_fpl : fpl, t_det>0 ? t_det : PLAT,  
	       tleft_geo,  tleft_ch, tright_geo, tright_ch, 
	       q_dev>0 ? q_dev : dev,
	       q_fpl>0 ? q_fpl : fpl, q_det>0 ? q_det : PLAQ,  
	       qleft_geo,  qleft_ch, qright_geo, qright_ch,
	       qtc_dev>0 ? qtc_dev : dev,
	       qtc_fpl>0 ? qtc_fpl : fpl, qtc_det>0 ? qtc_det : PLAT,  
	       qtcleft_geo,  qtcleft_ch, qtcright_geo, qtcright_ch,
	       mult_dev>0 ? mult_dev : dev,
	       mult_fpl>0 ? mult_fpl : fpl, mult_det>0 ? mult_det : PLAT,  
	       multleft_geo,  multleft_ch, multright_geo, multright_ch);
  ...
  pla_pmap.insert(std::pair<TArtRIDFMap, TArtPlasticPara *>(*((TArtRIDFMap *)para->GetQTCLMap()), para));
  pla_pmap.insert(std::pair<TArtRIDFMap, TArtPlasticPara *>(*((TArtRIDFMap *)para->GetQTCRMap()), para));
  pla_pmap.insert(std::pair<TArtRIDFMap, TArtPlasticPara *>(*((TArtRIDFMap *)para->GetMulHitTDCLMap()), para));
  pla_pmap.insert(std::pair<TArtRIDFMap, TArtPlasticPara *>(*((TArtRIDFMap *)para->GetMulHitTDCRMap()), para));
...
TArtICPara *TArtBigRIPSParameters::ParseICPara(TXMLNode *node) {
  ...
  Int_t geo[NUM_IC_CHANNEL], ch[NUM_IC_CHANNEL], t_geo[NUM_IC_CHANNEL], t_ch[NUM_IC_CHANNEL];
  char dummy[64];
  ...
  for ( ; node; node = node->GetNextNode()) {
	...
	sprintf(dummy,"t_geo%d",i);
	if (strcmp(node->GetNodeName(), dummy) == 0)
	  t_geo[i] = atoi(node->GetText());

	sprintf(dummy,"t_ch%d",i);
	if (strcmp(node->GetNodeName(), dummy) == 0)
	  t_ch[i] = atoi(node->GetText());

	sprintf(dummy,"tcal%d",i);
	if (strcmp(node->GetNodeName(), dummy) == 0)
	  tcal[i] = atoi(node->GetText());
  ...
  para->SetTcal(tcal);
  para->SetMap(det_adc, geo, ch, det_tdc, t_geo, t_ch);
...
TArtGePara *TArtBigRIPSParameters::ParseGePara(TXMLNode *node) {
  ...
  Int_t geo_adc = -1, ch_adc = -1;
  Int_t geo_tdc = -1, ch_tdc = -1;
...
TArtSiPara *TArtBigRIPSParameters::ParseSiPara(TXMLNode *node) {
  ...
  Double_t  zcoef_0 = 0, zcoef_1 = 0, zcoef_2 = 0;
  Double_t  ionpair = 0;
  Int_t geo_adc = -1, ch_adc = 0;
  Int_t geo_tdc = -1, ch_tdc = 0;
...
TArtTKEPara *TArtBigRIPSParameters::ParseTKEPara(TXMLNode *node) {
  ...
  Int_t det_adc = NAIE;
  Double_t t_ch2nsec = 0, t_offset = 0;
  Int_t fpl_tdc = -1, geo_tdc = -1, ch_tdc = -1;
  Int_t det_tdc = NAIT;
  ...
  Double_t  t_ch2nsecarray[NUM_TKE_CHANNEL], t_offsetarray[NUM_TKE_CHANNEL];
  Int_t geo_tdcarray[NUM_TKE_CHANNEL], ch_tdcarray[NUM_TKE_CHANNEL];
  for(int i=0;i<NUM_TKE_CHANNEL;i++){
    e_ch2mevarray[i] = 0; e_pedarray[i] = 0;
    geo_adcarray[i] = -1; ch_adcarray[i] = -1;
    t_ch2nsecarray[i] = 0; t_offsetarray[i] = 0;
    geo_tdcarray[i] = -1; ch_tdcarray[i] = -1;
  }
  ...
  for ( ; node; node = node->GetNextNode()) {
	...
	e_ped = (Double_t)atof(node->GetText());
      else if (strcmp(node->GetNodeName(), "t_ch2nsec") == 0)
        e_ch2mev = (Double_t)atof(node->GetText());
      else if (strcmp(node->GetNodeName(), "t_offset") == 0)
        e_ped = (Double_t)atof(node->GetText());
	...
	geo_adc = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "e_ch") == 0)
	ch_adc = atoi(node->GetText());
      else if (strcmp(node->GetNodeName(), "t_fpl") == 0)
	fpl_tdc = atoi(node->GetText());
      else if (strcmp(node->GetNodeName(), "t_det") == 0)
	det_tdc = atoi(node->GetText());
      else if (strcmp(node->GetNodeName(), "t_geo") == 0)
	geo_tdc = atoi(node->GetText());
      else if (strcmp(node->GetNodeName(), "t_ch") == 0)
	ch_tdc = atoi(node->GetText());
      else if (nodename.Contains("e_ch2mev") ||
	       nodename.Contains("e_ped")    ||
	       nodename.Contains("e_geo")    ||
	       nodename.Contains("e_ch")       ){
	for(int i=0;i<NUM_TKE_CHANNEL;i++){
      ...
      else if (nodename.Contains("t_ch2nsec") ||
	       nodename.Contains("t_offset")    ||
	       nodename.Contains("t_geo")    ||
	       nodename.Contains("t_ch")       ){
	for(int i=0;i<NUM_TKE_CHANNEL;i++){
	  sprintf(dummy,"t_ch2nsec%d",i);
	  if (strcmp(node->GetNodeName(), dummy) == 0)
	    t_ch2nsecarray[i] = (Double_t)atof(node->GetText());
	  sprintf(dummy,"t_offset%d",i);
	  if (strcmp(node->GetNodeName(), dummy) == 0)
	    t_offsetarray[i] = (Double_t)atof(node->GetText());
	  sprintf(dummy,"t_geo%d",i);
	  if (strcmp(node->GetNodeName(), dummy) == 0)
	    geo_tdcarray[i] = atoi(node->GetText());
	  sprintf(dummy,"t_ch%d",i);
	  if (strcmp(node->GetNodeName(), dummy) == 0){
	    ch_tdcarray[i] = atoi(node->GetText());
	    //numtdc = NUM_TKE_CHANNEL;
	  }
	}//for(int i=0;i<NUM_TKE_CHANNEL;i++){
      }//else if 
      else {
  ...
  if(-1 == fpl_tdc) fpl_tdc = fpl;      

  TArtCore::Info(__FILE__,"Reading database for %s TKE",name.Data());
  TArtTKEPara *para = NULL;
  if(numadc>1){
    para = new TArtTKEPara(id, name, fpl, e_ch2mevarray, e_pedarray, t_ch2nsecarray, t_offsetarray);
    for(int i=0;i<NUM_TKE_CHANNEL;i++){
      para->SetMap(i,fpl_adc, det_adc, geo_adcarray[i], ch_adcarray[i],fpl_tdc, det_tdc, geo_tdcarray[i], ch_tdcarray[i]);
      tke_pmap.insert(std::pair<TArtRIDFMap, TArtTKEPara *>(*((TArtRIDFMap *)para->GetADCMap(i)), para));
      tke_pmap.insert(std::pair<TArtRIDFMap, TArtTKEPara *>(*((TArtRIDFMap *)para->GetTDCMap(i)), para));
    }
  }
  else{
    para = new TArtTKEPara(id, name, fpl, e_ch2mev, e_ped, t_ch2nsec, t_offset);
    para->SetMap(fpl_adc, det_adc, geo_adc, ch_adc, fpl_tdc, det_tdc, geo_tdc, ch_tdc);
    tke_pmap.insert(std::pair<TArtRIDFMap, TArtTKEPara *>(*((TArtRIDFMap *)para->GetADCMap()), para));
    tke_pmap.insert(std::pair<TArtRIDFMap, TArtTKEPara *>(*((TArtRIDFMap *)para->GetTDCMap()), para));
  }
...
TArtFocalPlanePara *TArtBigRIPSParameters::ParseFocalPlanePara(TXMLNode *node) {
  ...
  for ( ; node; node = node->GetNextNode()) {
	...
	fpl = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "zpos") == 0)
	zpos = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(), "zshift") == 0)
	zshift = (Double_t)atof(node->GetText());
    }
  }
...
//__________________________________________________________                           
Double_t TArtBigRIPSParameters::GetDipoleNMR(TString *dname) {
  std::vector <Dipole>::iterator itr = dipoles.begin();
  while(itr != dipoles.end()){
    if(*dname == (*itr).dipolename) return (*itr).nmr;
    itr ++;
  }
  return -1;
}
