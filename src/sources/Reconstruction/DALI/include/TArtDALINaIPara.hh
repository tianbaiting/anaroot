...
class TArtDALINaIPara : public TNamed {
 ...
 public:
  TArtDALINaIPara(){}
  TArtDALINaIPara(Int_t i, TString n, Int_t f, Int_t l, Double_t tc, Double_t to, Double_t qc, Double_t qp, Double_t th, Double_t x, Double_t y, Double_t z) :
    id(i), detname(n), fpl(f), layer(l), tcal(tc), toff(to), qcal(qc), qped(qp), theta(th), x_pos(x),  y_pos(y), z_pos(z){ }
  ~TArtDALINaIPara() { }

  void SetParameters(Int_t i, TString n, Int_t f, Int_t l, Double_t tc, Double_t to, Double_t qc, Double_t qp, Double_t th, Double_t x, Double_t y, Double_t z) { id=i; detname=n; fpl=f; layer=l; tcal=tc; toff=to; qcal=qc; qped=qp; theta=th;  x_pos=x; y_pos=y; z_pos=z;}
  ...
  Double_t GetXPos() const { return x_pos; }
  Double_t GetYPos() const { return y_pos; }
  Double_t GetZPos() const { return z_pos; }
  Double_t GetNaITheta() const { return theta; }

  TArtRIDFMap * GetTDCMap() { return &map_tdc;  }
  TArtRIDFMap * GetADCMap() { return &map_adc;  }

  friend std::ostream & operator << (std::ostream& out, const TArtDALINaIPara& p) {
    out << "ID: " << p.id << ", ";
    ...
    out << "Position: " <<p.x_pos<< " " <<p.y_pos<< " " <<p.z_pos<< ", ";
