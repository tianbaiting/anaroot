...
TArtDALINaIPara *TArtDALIParameters::ParseDALINaIPara(TXMLNode *node) {
  ...
  for ( ; node; node = node->GetNextNode()) {
	...
	qped = (Double_t)atof(node->GetText());
      else if (strcmp(node->GetNodeName(), "theta") == 0)
	...
	adc_ch = atoi(node->GetText());
      ...
      // not used in TArtCalibDALI at this moment 2015/03/24 ->
      else if (strcmp(node->GetNodeName(), "ID") == 0) 
	;// do nothing
      else if (strcmp(node->GetNodeName(), "x_pos") == 0)
        x_pos = (Double_t)atof(node->GetText());
      else if (strcmp(node->GetNodeName(), "y_pos") == 0) 
        y_pos = (Double_t)atof(node->GetText());
      else if (strcmp(node->GetNodeName(), "z_pos") == 0) 
  ...
  TArtDALINaIPara * para = new TArtDALINaIPara(id, name, fpl, layer, tcal, toff, qcal, qped, theta, x_pos, y_pos, z_pos);
