...
class TArtDCHit : public TArtDataObject {
 ...
 public:
  ...
  virtual void Copy(TArtDCHit& hit) const {
    TArtDCHit &obj = *(TArtDCHit*) &hit;
    obj.SetTDC(fTDC);
    obj.SetTrailTDC(fTrailTDC);
    obj.SetPosition(fPosition);
    obj.SetWireDirection(anodedir);
    obj.SetWireID(wireid);
    obj.SetWirePosition(wirepos);
    obj.SetWireZPosition(wirez);
    obj.SetLayer(layer);
    obj.SetPlaneID(id_plane); 
    TArtDataObject::Copy(obj);
  }
