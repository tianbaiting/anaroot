...
//  tdcwindow[0] = 200; 
//  tdcwindow[1] = 2100; 
  tdcwindow[0] = 4000; // temp for MS2025 (2025.05.29)
  tdcwindow[1] = 4700; 
...
void TArtCalibDCTrack::SetTDCDistribution(TH1 * inhtdc, Int_t layer_id) {
  ...
  std::cout<<tdcwindow[0]<<" "<<tdcwindow[1]<<" "
	   <<inhtdc->FindBin(tdcwindow[0])<<" "<<inhtdc->FindBin(tdcwindow[1])<<" "
	   <<std::endl;
  std::cout<<"SetTDCDistribution tdcint="<<tdcint[layer_id]<<std::endl;
  if(tdcwindow[0]<0){ // in the case of v1190/v1290 case
	...
	tdc2mm[layer_id][i] = pitch[layer_id] * 0.5 * (Double_t)inhtdc->Integral(inhtdc->FindBin(tdcwindow[0]),inhtdc->FindBin(i))/tdcint[layer_id];
      }

    }
  }

  TArtCore::Info(__FILE__,"Set TDC Window: ",tdcwindow[0],tdcwindow[1]);
...
//    std::cout<<"tdcint="<<tdcint[layer_id]<<" "
//	     <<"layerid="<<layer_id<<" "
//	     <<std::endl;
