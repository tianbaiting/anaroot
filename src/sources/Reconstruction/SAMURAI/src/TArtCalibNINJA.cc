...
void TArtCalibNINJA::LoadData(TArtRawSegmentObject* seg)
{
  ...
  for(Int_t j=0; j<seg->GetNumData(); ++j){
    ...
    if(geo==2 && ch==127 && edge==0) fTimeReferenceV1190 = val;
	...
	if(pla->fT_LERaw[index]==-1) pla->fT_LERaw[index] = val;

      } else if(edge==1) {

	if(pla->fT_TERaw[index]==-1) pla->fT_TERaw[index] = val;
    ...
    // cout << "neko edge " << edge << " " << ch << " " << val << detector << " " << index << endl; 
    // NOTE: Hits starting from trailing edge are dropped.
  }
}
//__________________________________________________________
void TArtCalibNINJA::ReconstructData()
{
  ...
  for(Int_t i=0;i<GetNumNINJAPla();++i){
    ...
    pla->fT_LECal[0] = (pla->fT_LERaw[0]-fTimeReferenceV1190) * para->fT_LECal[0] + para->fT_LEOffset[0] - fTimeOrigin; 
    pla->fT_LECal[1] = (pla->fT_LERaw[1]-fTimeReferenceV1190) * para->fT_LECal[1] + para->fT_LEOffset[1] - fTimeOrigin; 
    pla->fT_TECal[0] = (pla->fT_TERaw[0]-fTimeReferenceV1190) * para->fT_TECal[0] + para->fT_TEOffset[0] - fTimeOrigin; 
    pla->fT_TECal[1] = (pla->fT_TERaw[1]-fTimeReferenceV1190) * para->fT_TECal[1] + para->fT_TEOffset[1] - fTimeOrigin; 


    pla->fT_LEAve = (pla->fT_LECal[0] + pla->fT_LECal[1])/2.0 ;
...
//__________________________________________________________
void TArtCalibNINJA::SetTimeOrigin(Double_t t0)
{
  fTimeOrigin = t0;
}
