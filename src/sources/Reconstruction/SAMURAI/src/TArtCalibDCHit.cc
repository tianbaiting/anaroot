...
TArtCalibDCHit::TArtCalibDCHit(const TString outdataname, const Int_t detid) 
  : TArtReconstruction(outdataname), detectorid(detid) 
{
...
void TArtCalibDCHit::LoadData()   {
  ...
  for(Int_t i=0;i<fEvent->GetNumSeg();i++){
    ...
    if((device == SAMURAI) && (detectorid == detector)){
      // Int_t nhit = fDCHitArray->GetEntries(); TArtCore::Info(__FILE__,"Entries in DCHitArray: %d", nhit);
      LoadData(seg);
    }
  }
  return;
