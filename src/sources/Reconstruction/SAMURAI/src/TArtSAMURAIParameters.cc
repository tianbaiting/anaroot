...
#define ERROR_PREFIX csvfile << ':' << line_no << ": "
  for (int line_no = 1;; ++line_no) {
    char line[256];
    bool is_start = false;
    int plane=-1, bar=-1, t_type=-1, channel=-1;
    double ns=-1;
...
TArtNEBULAPlaPara *TArtSAMURAIParameters::ParseNEBULAPlaPara(TXMLNode *node)
{
  ...
  for(; node; node = node->GetNextNode()){
    ...
    }else if(strcmp(node->GetNodeName(), "tu_det") == 0){
      tu_det = atoi(node->GetText());
    }else if(strcmp(node->GetNodeName(), "td_det") == 0){
      td_det = atoi(node->GetText());
    }else if(strcmp(node->GetNodeName(), "is_tref") == 0){
      Int_t val = atoi(node->GetText());
      para->SetIsTref(val);
    }else if(strcmp(node->GetNodeName(), "is_hpc") == 0){
      Int_t val = atoi(node->GetText());
      para->SetIsHPC(val);
    }else if(strcmp(node->GetNodeName(), "id_hpc") == 0){
      Int_t hpcid = atoi(node->GetText());
      para->SetHPCID(hpcid);
    }
...
//  }
  ...
  if(para->GetLayer() == 3){// for new config. with NEBULA Plus
    detector_q = NEBULA1Q;
  }else if(para->GetLayer() == 4){
    detector_q = NEBULA2Q;
  }

  para->SetMapQU(detector_q, qu_geo, qu_ch);
  fNEBULAPlaUParaMap->insert(TArtNEBULAPlaParaPair(para->GetMapQU(), para));
  //para->SetMapU(detector_t, tu_geo, tu_ch);
  para->SetMapTU(tu_det, tu_geo, tu_ch);
  fNEBULAPlaUParaMap->insert(TArtNEBULAPlaParaPair(para->GetMapTU(), para));
  para->SetMapQD(detector_q, qd_geo, qd_ch);
  fNEBULAPlaDParaMap->insert(TArtNEBULAPlaParaPair(para->GetMapQD(), para));
  //para->SetMapD(detector_t, td_geo, td_ch);
  para->SetMapTD(td_det, td_geo, td_ch);
  fNEBULAPlaDParaMap->insert(TArtNEBULAPlaParaPair(para->GetMapTD(), para));
...
TArtNEBULAPlaPara *TArtSAMURAIParameters::ParseNeuLANDVETOPlaPara(TXMLNode *node)
{
  ...
  para->SetMapTU(detector_t, tu_geo, tu_ch);
  fNeuLANDVETOPlaUParaMap->insert(TArtNEBULAPlaParaPair(para->GetMapTU(), para));
  para->SetMapTD(detector_t, td_geo, td_ch);
  fNeuLANDVETOPlaDParaMap->insert(TArtNEBULAPlaParaPair(para->GetMapTD(), para));

  para->SetMapQU(detector_q, qu_geo, qu_ch);
  fNeuLANDVETOPlaUParaMap->insert(TArtNEBULAPlaParaPair(para->GetMapQU(), para));
  para->SetMapQD(detector_q, qd_geo, qd_ch);
  fNeuLANDVETOPlaDParaMap->insert(TArtNEBULAPlaParaPair(para->GetMapQD(), para));
...
TArtHODPlaPara *TArtSAMURAIParameters::ParseHODPlaPara(TXMLNode *node)
{
  ...
  Int_t tdet=-1, tup_geo=-1, tup_ch=-1, tdown_geo=-1, tdown_ch=-1;
  Int_t qdet=-1, qup_geo=-1, qup_ch=-1, qdown_geo=-1, qdown_ch=-1;
...
TArtRPTOFPlaPara *TArtSAMURAIParameters::ParseRPTOFPlaPara(TXMLNode *node)
{
  ...
  Int_t tul_v775_geo=-1, tul_v775_ch=-1, tdr_v775_geo=-1, tdr_v775_ch=-1;
  Int_t tul_v1290_geo=-1, tul_v1290_ch=-1, tdr_v1290_geo=-1, tdr_v1290_ch=-1;
  Int_t qul_geo=-1, qul_ch=-1, qdr_geo=-1, qdr_ch=-1;
...
TArtTEDCsIPara *TArtSAMURAIParameters::ParseTEDCsIPara(TXMLNode *node)
{
  ...
  Int_t fpl_adc = 13, geo_adc=-1, ch_adc=-1;
