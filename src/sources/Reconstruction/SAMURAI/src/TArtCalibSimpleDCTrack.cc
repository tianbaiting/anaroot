...
TArtCalibSimpleDCTrack::TArtCalibSimpleDCTrack(const TString indata, 
				   const TString outdata, 
				   const char *lpattern,
				   const Int_t minnumhitlayer)
  : TArtReconstruction(outdata), 
    tdczero(0), driftv(0), 
    nlayer_x(0), nlayer_y(0), nlayer_u(0), nlayer_v(0) 
{
  ...
  fuDCangle = 30./180.*TMath::Pi();//---changed by liuhn
...
//__________________________________________________________
TArtCalibSimpleDCTrack::~TArtCalibSimpleDCTrack()  {
  ClearData();
  delete [] hits_buffer;
...
//__________________________________________________________
void TArtCalibSimpleDCTrack::ClearData()   {
...
//__________________________________________________________
void TArtCalibSimpleDCTrack::SetTDCDistribution(TH1 * inhtdc, Int_t layer_id) {

  tdcint[layer_id] = (Double_t)inhtdc->Integral(inhtdc->FindBin(tdcwindow[0]),
					inhtdc->FindBin(tdcwindow[1]));
  // std::cout<<"layer_id="<<layer_id<<";tdcint_layer="<<tdcint[layer_id]<<std::endl; 
...
//__________________________________________________________
void TArtCalibSimpleDCTrack::ReconstructData() { // call after the raw data are loaded
...
//__________________________________________________________
void TArtCalibSimpleDCTrack::SetHitsBuffer() {
...
//__________________________________________________________
Double_t TArtCalibSimpleDCTrack::CalcDriftLength(Int_t tdc, Int_t layer_id = -1) {
...
//
//__________________________________________________________
void TArtCalibSimpleDCTrack::RecoSimpleTrack(std::vector <TArtDCHit *> *hits,Int_t nl, Int_t dir){
...
//__________________________________________________________
void TArtCalibSimpleDCTrack::CalcSimpleTrack(Double_t pos[][2], Int_t nl, TArtDCTrack *tr, Int_t dir) {
...
//__________________________________________________________
void TArtCalibSimpleDCTrack::FindAssociatePos(TArtDCTrack *tr, std::vector <TArtDCHit *> *hits, Double_t *pos) {
...
//
//__________________________________________________________
void TArtCalibSimpleDCTrack::RecoTrack() {

  //________Keep the first 4 tracks, instead of 10,  with least chi2,  by liuhn
  Int_t ntrack_keep = 4; 
  ...
  CalcTrack(hits_buffer_x, "X",nlayer_x, &tracks_x);
  ...
  if(nlayer_y>0){
    CalcTrack(hits_buffer_y, "Y",nlayer_y, &tracks_y);
  ...
  if(nlayer_u>0){
    CalcTrack(hits_buffer_u, "U",nlayer_u, &tracks_u);
  ...
  if(nlayer_v>0){
    CalcTrack(hits_buffer_v, "V",nlayer_v, &tracks_v);
...
//
//__________________________________________________________
void TArtCalibSimpleDCTrack::CalcTrack(std::vector <TArtDCHit *> *hits, TString layer_name,Int_t numlayer, TObjArray *tracks) {
  ...
  if(layer_name=="U" && nhitlayer<2) return;
  if((layer_name=="X"||layer_name=="Y"||layer_name=="V")&&minhitlayer>=0 && nhitlayer<minhitlayer) return; // return if num of hit layer is small.
...
TArtCalibSimpleDCTrack::Track1D * TArtCalibSimpleDCTrack::CalcTrack1D(std::vector <TArtDCHit *> *hits) {
  ...
  //Only use the wire position, without left and right ambigulty, by liuhn
  Int_t npattern=nl;
  ...
  for(Int_t i=0;i<npattern;i++){
    ...
    //Comment out, so that we only use wire position, by liuhn
	//xpos[l] += sign*dl[l]; 
...
//__________________________________________________________
TArtDCTrack * TArtCalibSimpleDCTrack::CalcTrackWith1DTracks(Track1D *trx, Track1D *tru, Track1D *trv) {

  TMatrixD rvec(4,1); rvec.Zero();
...
//__________________________________________________________
TArtDCTrack * TArtCalibSimpleDCTrack::GetDCTrack(Int_t i) {
  return (TArtDCTrack *)fDCTrackArray->At(i);
}
//__________________________________________________________
Int_t TArtCalibSimpleDCTrack::GetNumDCTrack() {
  return fDCTrackArray->GetEntries();
}
//__________________________________________________________
Int_t TArtCalibSimpleDCTrack::GetNumDCHit(Int_t l) {
  return hits_buffer[l].size();
}

//________________________________________________________________________
TArtCalibSimpleDCTrack::Track1D& TArtCalibSimpleDCTrack::Track1D::operator=(const TArtCalibSimpleDCTrack::Track1D &right){
...
ClassImp(TArtCalibSimpleDCTrack)
