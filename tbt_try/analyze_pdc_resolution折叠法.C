...
void analyze_pdc_resolution(const char* inrootfile = "./output/data0074.ridf_pdc_tdc_tot.root") {
    ...
    // --- 步骤 2 (严谨版): 分析空间分辨率 sigma_U (使用折叠法) ---
    
    std::cout << "开始分析空间分辨率 (sigma_U) 使用修正的折叠法..." << std::endl;
    TH1F *h_U_folded = new TH1F("h_U_folded", "Folded U distribution at min TDC; Folded U [mm]; Entries", 100, -6, 6);
    ...
    double period_U = 12.0;

    // 添加一个大的偏移量，确保fmod处理的是正数
    double offset = 1000.0 * period_U; 

    for(long long i=0; i<n_entries; ++i){
        tin->GetEntry(i);

        if(tree_mintdc_forU > tdc_min_cut && tree_mintdc_forU < tdc_max_cut){
            // *** 这是修正后的折叠公式 ***
            double U_folded = fmod(tree_U + offset + period_U/2.0, period_U) - period_U/2.0;
            h_U_folded->Fill(U_folded);
        }
    }

    // 拟合折叠后的单峰分布
    h_U_folded->Fit("gaus", "R");

    // 绘制拟合结果
    TCanvas *c_sres_folded = new TCanvas("c_sres_folded", "Folded Spatial Resolution at min TDC", 800, 600);
    c_sres_folded->cd();
    h_U_folded->Draw();

    TF1* fit_sres_folded = h_U_folded->GetFunction("gaus");
    if(fit_sres_folded){
        double spatial_res = fit_sres_folded->GetParameter(2); // sigma
        double err_spatial_res = fit_sres_folded->GetParError(2);
        std::cout << "--------------------------------------------------------" << std::endl;
        printf("估算的本征空间分辨率 (sigma_U, 折叠法) = %.3f +/- %.3f mm\n", spatial_res, err_spatial_res);
        std::cout << "--------------------------------------------------------" << std::endl;
    }
